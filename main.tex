...
secure and efficient runtime environment[1] 
that provides “write once, run anywhere” deployment 
\& maintenance of software applications 
in a large variety of programming languages[2]. 
While research into WebAssembly-based audio plugins is mature[3][4][5], 
...
with an emphasis on IoMusT[6] applications. 
\end{abstract}

\section{ Introduction }
\label{sec:intro}

The landscape of audio software development in 2026 is a notably diverse one.
Targets include:
...
in a live performance context, ideally on hardware that is better 
suited to that use case than a laptop. For developers, deployment
...
logic, without handling memory management etc.
...
connected by data flow lines, which mirrors the analog interface
of signal processing device composition familiar to musicians and 
...
that makes difficult many techniques.
