...
The use of third-party “plugin” processing is nearly ubiquitous in desktop audio software, but remains rare in embedded systems. Driven by the vast heterogeneity of embedded devices, siloed platforms have become the norm, a paradigm in which each hardware manufacturer maintains proprietary digital signal processing (DSP) modules that are incompatible, even when offering many of the same algorithms to end users. 

To enable cross-platform execution of third-party DSP in embedded systems, this paper proposes the distribution of audio DSP modules in a hardware-agnostic intermediate representation (IR). Drawing on Internet of Things (IoT) research, I've investigated the WebAssembly (WASM) binary instruction format as a candidate IR for portable, third-party audio processing. 

To assess feasibility under real-time constraints, I've compiled representative audio DSP algorithms to WASM using a consistent application binary interface (ABI) and compared their performance across three WASM runtimes on an embedded platform (Daisy Seed). To support further development, I've created an ecosystem of open-source tools for developing and deploying headless WASM-based plugins.

Performance measurements demonstrate that runtimes leveraging ahead-of-time (AOT) compilation yield acceptable performance for a meaningful subset of audio algorithms, indicating the viability of WASM as a portable distributable for embedded DSP. In addition to embedded deployment, WASM supports runtimes in desktop audio, plugin hosts, and web environments, enabling shared DSP implementations and presets across platforms.
...
The landscape of audio software in 2026 is diverse. Deployments include standalone applications spanning various operating systems and computer architectures, myriad plugin formats for digital audio workstations (DAWs), game engine plugins, web audio, embedded systems, and mobile apps.
...
In order to facilitate deployment to multiple targets from a single codebase, many solutions have arisen within the audio developer community. Intermediary plugin frameworks and domain specific languages (DSLs) for audio are the primary examples. While these tools each can each address many deployments from the same source code, none provide a unified distributable.

Single-distributable processing can confer many advantages, but carries with it some inherent downsides. Advantages include simplified build systems compared to platforms that generate native machine code for multiple targets, easier porting of legacy codebases to new platforms, and greater interoperability between heterogeneous deployments. Downsides include performance degradation from the abstraction layer, increased complexity of debugging, and easier de-compilation of the distributable. In this paper I investigate the targeting of embedded systems from such a single-distributable, as these resource-contained environments provide the greatest challenge in portability. If deployment to embedded systems proves feasible, then all other targets may follow suit with relative ease.
...
Ecosystem siloing introduces several practical limitations. Most notable among them:

\begin{itemize}
    \item Individual algorithm quality may suffer because hardware vendors are responsible for implementing and maintaining the broad range of effects that users expect, often redundantly \textit{re-}implementing the popular modules offered by similar products.
    
    \item Users cannot reliably maintain sonic consistency between desktop production environments and embedded devices, as the robust world of third-party plugin software for desktop audio is not available in embedded contexts.
    
    \item Shifts in popular audio processing techniques create ongoing maintenance burdens. When new paradigms emerge, manufacturers must issue firmware updates or redesign internal processing pipelines to remain competitive.
\end{itemize}
...
In computing generally, intermediate representation often takes one of two forms. Some forms, such as LLVM IR\cite{1281665}, are used internally by compilers as a translation stage in the process of turning high-level, human-readable source code into architecture-specific machine code prior to program execution. Another form of intermediate representation is \textit{bytecode}, which is also translated from higher-level source code but is intended to run inside a \textit{virtual machine}\cite{lindholm2013java}, a software program that translates intermediate representations to native machine code at runtime. Programming languages that compile to bytecode can use a single distributable to target any host with a corresponding runtime. 
 
 Recent Internet of Things (IoT) research\cite{fi15080275}\cite{10756513}\cite{IOTW753} has explored WebAssembly (WASM) as an intermediate representation of both forms described above, with the general goal of WASM as a “write once, run anywhere” substrate for sandboxed applications. 

% When used one way, it's interpretation, another way, AOT, 
 
In the audio field, a set of researchers have proposed WASM deployment as a universal distributable for audio DSP with a framework entitled \textit{Audio Anywhere}\cite{gaster2020audio}, although development of this project has ceased. In their work, they explored runtime interpretation and ahead-of-time (AOT) translation strategies for their runtimes targeting embedded, and limited their scope to WASM modules compiled from a single input source language. Building on this research, I've investigated alternative ahead-of-time (AOT) translation strategies for WASM modules, enabled additional high-level source code languages as input, and benchmarked a wider range of audio processing algorithms.
...
% To assess the viability of my approach, I've measured the performance of three WASM runtime strategies and compared against native execution on an embedded target representative of low-cost digital audio hardware: the Daisy Seed microcontroller platform. The selected DSP algorithms span major paradigms in contemporary audio effects design, including white-box, black-box, and physical modeling. 

Inline with the measurements taken in \cite{gaster2020audio}, my benchmark results indicate that runtime interpretation of WASM is not a viable strategy for real-time embedded audio workloads. However, runtimes leveraging ahead-of-time (AOT) translation of WASM modules yield acceptable performance characteristics when certain conditions are met, notably when dynamic memory allocation is not used. Performance may be further improved by utilizing WASM's interface for importing optimized math functions from its host, investigating alternative compiler toolchains, and retooling how runtimes handle dynamic memory. Notably, one of the AOT methods I utilize does not depend on local development tools, as I've embedded it in a client-side web application.

My findings suggest that with further research, a carefully specified WebAssembly ABI for audio processing and parameter management could enable language-agnostic development of encapsulated audio DSP coupled with deployment across embedded devices, desktop applications, and web platforms from a single distributable. In that aim, the primary contributions of this work are the exploration of multiple translation strategies, empirical benchmarking of their relative performance on representative embedded hardware, and creation of developer tooling to facilitate growth of this paradigm.
...
To investigate the feasibility of WebAssembly (WASM) as a universal distributable for embedded audio applications, I've compiled four representative audio processing routines to WASM with a consistent binary interface using Emscripten\cite{zakai2011emscripten}, and recorded timing benchmarks of each across three methods of executing the programs on a resource contained target. For a baseline reference, I compiled and benchmarked each of the four routines as native ARM code as well.
...
The FAUST domain-specific programming language is popular among audio developers for its capability to generate code for myriad targets to be compiled separately, as well as its high-level semantics for describing audio DSP. The maintainers of FAUST supply many example programs with the web-based IDE they provide\cite{ren2019faust}, including a the reverb algorithm I've used here. The algorithm uses physical modeling techniques to emulate the sounds of a spring reverb.

% \bigskip  

\subsubsection{ Gen\textasciitilde \space Phaser }

Gen\textasciitilde \space is another domain-specific programming language (DSL) for audio that is very popular. A major difference from FAUST is that Gen\textasciitilde \space is a \textit{visual} programming language, providing a user interface that can be less of a challenge for beginners. Like many other audio DSLs, Gen\textasciitilde \space provides the ability to export code to target different platforms. For the purposes of benchmarking, I constructed an elementary phaser effect in using four cascaded state variable filters (from the GO library\cite{BD0831917X}) and a low-frquency oscillator. 
...
My experimental results demonstrate that WebAssembly can support real-time audio DSP on embedded hardware under certain conditions, but further research is needed to determine the bounds of its performance. 
...
This work investigated the use of WebAssembly as a hardware-agnostic intermediate representation for distributing audio DSP modules to embedded targets. By benchmarking three runtime strategies on a representative embedded platform, I've evaluated the practicality of enabling third-party processing in resource-constrained audio systems.

My experimental results indicate that interpreter-based execution is not suitable for real-time embedded DSP. However, translation-based approaches demonstrate performance levels that are acceptable for a meaningful subset of audio algorithms. 
...
These findings support the broader claim that a carefully specified WebAssembly ABI for headless audio plugins could form the basis of a portable DSP distribution model, similar to that proposed in []. Such a model would allow algorithms written in multiple programming languages to compile to a single intermediate format, and to be uniformly deployed across embedded devices, desktop applications, and web platforms using appropriate runtimes.
